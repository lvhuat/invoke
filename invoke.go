...
type Invoker struct {
	...
	payload               func() (*bytes.Reader, error)
	client                *http.Client
...
func (invoker *Invoker) debugLogger(d time.Duration, req *http.Request, rsp *http.Response, err error) {
	...
	func() {
		...
		if err != nil {
			if req == nil {
				fmt.Fprintf(buf, "[ERRO][%s.%d]InvokeFailed\terror=%v\n", t, ms, err)
				return
			}

			fmt.Fprintf(buf, "[ERRO][%s.%d]InvokeFailed\t cost=%v url=%s error=<%v> header=%v\n", t, ms, d, req.URL.String(), err, req.Header)
		...
		if statusCode != 200 {
			fmt.Fprintf(buf, "[ERRO][%s.%d]InvokeDone\t cost=%v url=%s header=%v status=%d\n", t, ms, d, req.URL.String(), req.Header, statusCode)
		} else {
			fmt.Fprintf(buf, "[INFO][%s.%d]InvokeDone\t cost=%v url=%s header=%v status=%d\n", t, ms, d, req.URL.String(), req.Header, statusCode)
		}
	}()
...
func (invoker *Invoker) Payload(body []byte) *Invoker {
	invoker.payload = func() (*bytes.Reader, error) {
...
func (invoker *Invoker) Json(v interface{}) *Invoker {
	invoker.payload = func() (*bytes.Reader, error) {
		...
		if err != nil {
			return nil, &InvokeError{"MarshalJson", err, false, false}
		}
